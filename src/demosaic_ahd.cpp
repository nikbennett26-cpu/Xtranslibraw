...
// AHD algorithm: RawTherapee implementation (GPL-3).
//
// Adapts RawTherapee's ahd_demosaic to our standalone CFA interface:
//   1. build float rawData[h][w] (black-subtracted, scaled to RT's 0..65535),
//   2. provide an FC() shim from our cfa[4],
//   3. call the ported routine (threading the optional rgb_cam matrix), and
...
int demosaic_ahd(const uint16_t* mosaic, int w, int h, const int cfa[4],
    ...
    FcCtx ctx{cfa};
    ahd_demosaic(w, h, rawRows.data(), redRows.data(), greenRows.data(),
                 blueRows.data(), rgb_cam, &fc_shim, &ctx);
...
#else  // HAVE_AHD not wired — fall back to bilinear
