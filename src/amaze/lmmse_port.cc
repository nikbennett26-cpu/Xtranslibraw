...
void RawImageSource::lmmse_interpolate_omp(int winw, int winh, const array2D<float> &rawData, array2D<float> &red, array2D<float> &green, array2D<float> &blue, int iterations)
{
    ...
    // Standalone build: no RT Color gamma tables. Run LMMSE in linear via an
    // identity LUT (also normalises the 0..65535 raw to 0..1 on lookup).
    ...
    // Inverse of the identity forward LUT (maps 0..1 * 65535 back to 0..65535).
    gamtab->makeIdentity();
...
// ---------------------------------------------------------------------------
// Standalone free-function wrapper (see lmmse_standalone.h).
// ---------------------------------------------------------------------------
...
{
    // Wrap the caller's row buffers without copying (~100 MB/plane at full res).
    array2D<float> rawA(width, height, const_cast<float**>(rawData), ARRAY2D_BYREFERENCE);
    array2D<float> redA(width, height, red,   ARRAY2D_BYREFERENCE);
    array2D<float> greenA(width, height, green, ARRAY2D_BYREFERENCE);
    array2D<float> blueA(width, height, blue,  ARRAY2D_BYREFERENCE);

    rtengine::RawImageSource src(width, height, rawA, redA, greenA, blueA);
    src.setCfa(fcfn(0, 0, fcctx), fcfn(0, 1, fcctx),
               fcfn(1, 0, fcctx), fcfn(1, 1, fcctx));

    // iterations=2: core LMMSE plus one median-refinement pass (iter=1), run in
    // linear (identity gamma). >4 would invoke RT's refinement(), removed below.
    src.lmmse_interpolate_omp(width, height, rawA, redA, greenA, blueA, 2);
}

// RT's post-refinement pass operates on RawImageSource member planes (red/green/
// blue) that the standalone wrapper does not carry, and is only reachable for
// iterations>4 (never requested here). Disabled to keep the shim surface minimal.
#if 0
namespace rtengine {
