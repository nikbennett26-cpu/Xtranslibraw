...
// IGV (Integrated Gaussian Vector on Color Differences) demosaic.
// Copyright (c) 2007-2013 Luis Sanz Rodriguez; RawTherapee adaptation by
// Jacques Desmis (3/2013); SSE version by Ingo Weyrich (5/2013).  GPL-3.
//
// Vendored SSE variant of RawImageSource::igv_interpolate, extracted verbatim
// from RawTherapee 5.11 rtengine/demosaic_algos.cc (lines 218-607). Includes
// swapped to the standalone shims; numeric body unchanged. Compiled with
// __SSE2__ (emcc -msse2 -msimd128), so only the SSE path is required.
#include <cmath>
#include <cstdlib>
...
// ---------------------------------------------------------------------------
// Standalone free-function wrapper (see igv_standalone.h).
// ---------------------------------------------------------------------------
void igv_demosaic(int width, int height,
                  float* const* rawData,
                  float** red, float** green, float** blue,
                  int (*fcfn)(int, int, void*), void* fcctx)
{
    // Wrap the caller's row buffers without copying.
    ...
    src.igv_interpolate(width, height);
}
