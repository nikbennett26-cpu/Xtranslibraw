...
// Original AHD by Keigo Hirakawa & Thomas Parks; RawTherapee implementation
// (GPL-3). Vendored copy of RawImageSource::ahd_demosaic from RawTherapee 5.11
// rtengine/ahd_demosaic_RT.cc (lines 44-232). Includes swapped to the
// standalone shims and the BENCHFUN timing macro removed; numeric body
// unchanged. The CIELab conversion is self-contained (inline XYZ matrix); the
// only external dependency is imatrices.rgb_cam, injected via setRgbCam().
#include <cstdint>
#include <cmath>
#include "rtengine.h"
...
// ---------------------------------------------------------------------------
// Standalone free-function wrapper (see ahd_standalone.h).
// ---------------------------------------------------------------------------
void ahd_demosaic(int width, int height,
                  float* const* rawData,
                  float** red, float** green, float** blue,
                  const float* rgb_cam,   // row-major 3x4 camera->linear, nullable
                  int (*fcfn)(int, int, void*), void* fcctx)
{
    ...
    src.setRgbCam(rgb_cam);   // no-op (identity default) when rgb_cam == nullptr
