...
//
// Adapts RawTherapee's rcd_demosaic to our standalone CFA interface:
...
int demosaic_rcd(const uint16_t* mosaic, int w, int h, const int cfa[4],
    ...
    FcCtx ctx{cfa};
    rcd_demosaic(w, h, rawRows.data(), redRows.data(), greenRows.data(),
...
#else  // HAVE_RCD not wired — fall back to bilinear
